...
export class CogbytesSearchComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.showAll == true) {
      ...
      this.cogbytesService.getTask().subscribe((data: any) => {
        this.taskList = data;
      });
      this.cogbytesService.getSpecies().subscribe((data: any) => {
        this.specieList = data;
      });
      this.cogbytesService.getSex().subscribe((data: any) => {
        this.sexList = data;
      });
      this.cogbytesService.getStrain().subscribe((data: any) => {
        this.strainList = data;
      });
      this.cogbytesService.getGenos().subscribe((data: any) => {
        this.genoList = data;
      });
      this.cogbytesService.getAges().subscribe((data: any) => {
        this.ageList = data;
      });
    }
  ...
  getFileType(upload: any) {
    const fileType: string = this.fileTypeList[this.fileTypeList.map(function (x: any) {
      return x.id;
    }).indexOf(upload.fileTypeID)].fileType;
    return fileType;
  }
  ...
  getUploadTaskString(upload: any) {
    let taskString = '';
    for (const id of upload.taskID) {
      taskString += this.taskList[this.taskList.map(function (x: any) {
        return x.id;
      }).indexOf(id)].task + ', ';
    }
  ...
  getUploadSpeciesString(upload: any) {
    let speciesString = '';
    for (const id of upload.specieID) {
      speciesString += this.specieList[this.specieList.map(function (x: any) {
        return x.id;
      }).indexOf(id)].species + ', ';
    }
  ...
  getUploadSexString(upload: any) {
    let sexString = '';
    for (const id of upload.sexID) {
      sexString += this.sexList[this.sexList.map(function (x: any) {
        return x.id;
      }).indexOf(id)].sex + ', ';
    }
  ...
  getUploadStrainString(upload: any) {
    let strainString = '';
    for (const id of upload.strainID) {
      strainString += this.strainList[this.strainList.map(function (x: any) {
        return x.id;
      }).indexOf(id)].strain + ', ';
    }
  ...
  getUploadGenoString(upload: any) {
    let genoString = '';
    for (const id of upload.genoID) {
      genoString += this.genoList[this.genoList.map(function (x: any) {
        return x.id;
      }).indexOf(id)].genotype + ', ';
    }
  ...
  getUploadAgeString(upload: any) {
    let ageString = '';
    for (const id of upload.ageID) {
      ageString += this.ageList[this.ageList.map(function (x: any) {
        return x.id;
      }).indexOf(id)].ageInMonth + ', ';
    }

...
import {
  DOINOTVALID, FIELDISREQUIRED, PUBLICATIONEDITFAILED, PUBLICATIONEDITSUCCESSFULL,
  PUBLICATIONSUCESSFULLYADDED, PUBLICATIONWITHSAMEDOI, PUBMEDKEYNOTVALID, YEARNOTVALID
...
export class PubscreenDialogeComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.pubScreenService.getTask().subscribe((data: any) => {
      this.taskList = data; this.processList(this.taskList, 'None', 'task');
    });
    this.pubScreenService.getSpecie().subscribe((data: any) => {
      this.specieList = data; this.processList(this.specieList, 'Other', 'species');
    });
    ...
    if (this.data.publicationObj != null) {
      ...
      this.pubScreenService.getPaperInfo(this.publicationId).subscribe((data: any) => {
        ...
        // });
        this.subSubTaskList = this.cognitiveTaskModel && this.subTaskList ? this.subTaskList.filter((x: any) =>
          this.cognitiveTaskModel.includes(x.taskID)) : [];
        ...
        this.subSubModelList = this.diseaseModel && this.subModelList ? this.subModelList.filter((x: any) =>
          this.diseaseModel.includes(x.modelID)) : [];
        this.filteredSubModelList.next(this.subSubModelList.slice());
        this.subSubMethodList = this.methodModel && this.subMethodList ? this.subMethodList.filter((x: any) =>
          this.methodModel.includes(x.methodID)) : [];
        this.filteredSubMethodList.next(this.subSubMethodList.slice());
        this.subSubRegionList = this.regionModel && this.subRegionList ? this.subRegionList.filter((x: any) =>
          this.regionModel.includes(x.rid)) : [];
  ...
  isOtherStrainMouse() {
    if (this.strainList !== undefined) {
      ...
      const otherIndex = this.strainList.find((x: any) => x.strain === 'Other (mouse)').id;
  ...
  isOtherStrainRat() {
    if (this.strainList !== undefined) {
      ...
      const otherIndex = this.strainList.find((x: any) => x.strain === 'Other (rat)').id;
      if (this.strainModel.includes(otherIndex)) {
        return true;
      }
    }
  ...
  addDOI(doi: string) {
    this.pubScreenService.getPaparInfoFromDOI(doi).subscribe((data: any) => {
      ...
      const apiResult = 'result' in data ? data.result : data;
  ...
  addEditPublication() {
    ...
    if (this.isEditMode) {
      if (this.isPublicMode) {
        this.pubScreenService.editPublicationPublic(this.publicationId, this._pubscreen).subscribe((data: any) => {
        ...
        this.pubScreenService.editPublication(this.publicationId, this._pubscreen).subscribe((data: any) => {

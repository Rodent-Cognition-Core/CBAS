            ...
            using (DataTable dt = Dal.GetDataTable($@"Select * From mbr.UserRepository Where Username='{userEmail}' Order By DateRepositoryCreated"))
            {
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    RepList.Add(new Cogbytes
                    {
                        ...
                        MethodID = FillCogbytesItemArray($"Select MID From mbr.DatasetMethod Where RepID={repID}", "MID"),
                        SubMethodID = FillCogbytesItemArray($"Select SMID From mbr.DatasetSubMethod Where RepID={repID}", "SMID"),
            ...
            using (DataTable dt = Dal.GetDataTable(sql))
            {
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    Uploadlist.Add(new CogbytesUpload
                    {
                        ...
                        TaskID = FillCogbytesItemArray($"Select TaskID From mbr.DatasetTask Where RepID={Int32.Parse(dr["RepID"].ToString())}", "TaskID"),
                        SpecieID = FillCogbytesItemArray($"Select SpeciesID From mbr.DatasetSpecies Where RepID={Int32.Parse(dr["RepID"].ToString())}", "SpeciesID"),
                        SexID = FillCogbytesItemArray($"Select SexID From mbr.DatasetSex Where RepID={Int32.Parse(dr["RepID"].ToString())}", "SexID"),
                        StrainID = FillCogbytesItemArray($"Select StrainID From mbr.DatasetStrain Where RepID={Int32.Parse(dr["RepID"].ToString())}", "StrainID"),
                        GenoID = FillCogbytesItemArray($"Select GenoID From mbr.DatasetGeno Where RepID={Int32.Parse(dr["RepID"].ToString())}", "GenoID"),
                        AgeID = FillCogbytesItemArray($"Select AgeID From mbr.DatasetAge Where RepID={Int32.Parse(dr["RepID"].ToString())}", "AgeID"),
